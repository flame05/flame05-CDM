...
$('.cdm-mod-parte').click(function(){
	id = $(this).attr('data-parte-id');
	$.getJSON('index.php?page=admin&parteupdate=1&id='+id, function(data){
		$('#modPid').val(data.id);
		$('#modPnome').val(data.nome);
		$('#modPordine').val(data.ordine);
	});

	$('#modParte').modal('toggle');
});
